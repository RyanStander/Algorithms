 ...
 class HighLevelDungeonNodeGraph : NodeGraph
{
    ...
    //FOREACH room
        //add nodes to all rooms
    //END FOREACH
    //FOREACH door
        //add nodes to all doors
        //add connection from each room to door thats connected to it
    //END FOREACH
	...
	protected override void generate ()
	{
        ...
        {
            nodes.Add(new Node(getDoorCenter(door)));
            AddConnection(nodes[door.roomA], nodes[nodes.Count-1]);

            AddConnection(nodes[door.roomB], nodes[nodes.Count - 1]);
        }  
	}
